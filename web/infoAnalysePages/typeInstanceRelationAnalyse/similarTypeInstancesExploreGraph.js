...
$(document).ready(function() {
    ...
    var sourceDataInstanceId=getQueryString("sourceDataInstanceId");
	var dimensionsIdList=getQueryString("dimensionsIdList");
	var filteringPattern=getQueryString("filteringPattern");
    var graphHeight=getQueryString("graphHeight");
    if(!discoverSpaceName){return;}
    if(!sourceDataInstanceId){return;}
	if(!dimensionsIdList){return;}
	if(!filteringPattern){return;}
    if(graphHeight){
        document.getElementById('mynetwork').style.height=""+graphHeight+"px";
    }
    sourceDataInstanceId=sourceDataInstanceId.replace(/#/g, "%23");
    sourceDataInstanceId=sourceDataInstanceId.replace(/:/g, "%3a");
    ...
    $.ajax({
		...
		var similarTypeInstancesDetailInfoList=data.similarTypeInstancesDetailInfoList;
		if(similarTypeInstancesDetailInfoList.length==0){
			return;
		}
		var sourceTypeInstanceDetailInfo=data.sourceTypeInstanceDetailInfo;
		var sourceTypeInstance=sourceTypeInstanceDetailInfo.measurableInstanceDetailInfo;
		var sourceTypeInstanceId=sourceTypeInstance.measurableId;
        var sourceTypeInstanceLabelName=sourceTypeInstance.measurableName;
        if(sourceTypeInstance.measurableAliasName){
            sourceTypeInstanceLabelName=sourceTypeInstance.measurableAliasName;
        }
        nodesDataArray.push(
            {
                id: sourceTypeInstanceId,
                label: sourceTypeInstanceLabelName+"["+sourceTypeInstanceId+"]",
                group: sourceTypeInstance.measurableType,
                color: BLACK,
                shape: dataInstanceTypeShapeMap[sourceTypeInstance.measurableType],                
 				title:getDetailTitleForMeasurable(sourceTypeInstance)
            }
        ); 

		var sourceTypeInstanceRelations=sourceTypeInstanceDetailInfo.relationsDetailInfoList;
        $.each(sourceTypeInstanceRelations,function(index,value){
		...
		$.each(similarTypeInstancesDetailInfoList,function(index,value){		
			var currentSimilarTypeInstance=value.measurableInstanceDetailInfo;
			var currentSimilarTypeInstanceId=currentSimilarTypeInstance.measurableId;
		    var currentSimilarTypeInstanceLabelName=currentSimilarTypeInstance.measurableName;
		    if(currentSimilarTypeInstanceLabelName.measurableAliasName){
		        currentSimilarTypeInstanceLabelName=currentSimilarTypeInstanceLabelName.measurableAliasName;
		    }
		    nodesDataArray.push(
		        {
		            id: currentSimilarTypeInstanceId,
		            label: currentSimilarTypeInstanceLabelName+"["+currentSimilarTypeInstanceId+"]",
		            group: currentSimilarTypeInstance.measurableType,
		            color: getCurrentGlobalColor(currentExploreLevel),
		            shape: dataInstanceTypeShapeMap[currentSimilarTypeInstance.measurableType],                
	 				title:getDetailTitleForMeasurable(currentSimilarTypeInstance)
		        }
		    ); 

			var currentSimilarTypeInstanceRelations=value.relationsDetailInfoList;
		    $.each(currentSimilarTypeInstanceRelations,function(index,value){
		        var relationId=value.id;
		        var relationTypeName=value.relationTypeName;
		        var fromRelationable=value.fromRelationable;
		        var toRelationable=value.toRelationable;
